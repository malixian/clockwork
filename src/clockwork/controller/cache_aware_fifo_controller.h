...
/* Cache-aware Multi Machine, Multi GPU FIFO Controller, 
if the weights are already loaded on a GPU, it sends the requests to that machine, 
otherwise loads the weight on the next available worker/gpu in a round-robin manner
*/
class CFifoControllerImpl : public Controller {
public:
	...
	std::map<int, std::pair<unsigned, unsigned> > cached_models_stat; 
	std::map<int, std::atomic_int> load_model_tracking;
	unsigned slo;

	CFifoControllerImpl(int client_port, std::vector<std::pair<std::string, std::string>> worker_host_port_pairs, unsigned slo);

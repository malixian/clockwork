...
RandomControllerImpl::RandomControllerImpl(int client_port, std::vector<std::pair<std::string, std::string>> worker_host_port_pairs) : Controller(client_port, worker_host_port_pairs), model_id_seed(0), action_id_seed(0) {
	current_worker = 0;
...
void RandomControllerImpl::save_callback(int id, std::function<void(std::shared_ptr<workerapi::Result>)> callback) {
...
std::function<void(std::shared_ptr<workerapi::Result>)> RandomControllerImpl::get_callback(int id) {
	std::lock_guard<std::mutex> lock(actions_mutex);
	auto it = action_callbacks.find(id);
...
void RandomControllerImpl::sendResult(std::shared_ptr<workerapi::Result> result) {
	auto callback = get_callback(result->id);
	CHECK(callback != nullptr) << "Received result without a corresponding action: " << result->str();
	callback(result);
}

void RandomControllerImpl::uploadModel(clientapi::UploadModelRequest &request, std::function<void(clientapi::UploadModelResponse&)> callback) {
...
void RandomControllerImpl::infer(clientapi::InferenceRequest &request, std::function<void(clientapi::InferenceResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
	...
	int target = rand() % (workers.size() * GPUS_PER_WORKER);
	current_worker = target / workers.size();
	unsigned gpu_id = target % GPUS_PER_WORKER;

	// If weights aren't loaded, send a load_weights action
	if (new_weights_available_at.find(std::make_tuple(request.model_id, current_worker, gpu_id)) == new_weights_available_at.end() ) {
		...
		new_weights_available_at[std::make_tuple(model_id, current_worker, gpu_id)] = util::now() + 7000000UL; // Assume it'll take 7 ms to load weights
	}
	...
	infer->earliest = new_weights_available_at[std::make_tuple(model_id, current_worker, gpu_id)];
	...
	std::cout << "<<<<< worker: " << current_worker << "  gpu: " << gpu_id << " >>>> \n";
	workers[current_worker]->sendActions(actions);
}

/** This is a 'backdoor' API function for ease of experimentation */
void RandomControllerImpl::evict(clientapi::EvictRequest &request, std::function<void(clientapi::EvictResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
...
/** This is a 'backdoor' API function for ease of experimentation */
void RandomControllerImpl::loadRemoteModel(clientapi::LoadModelFromRemoteDiskRequest &request, std::function<void(clientapi::LoadModelFromRemoteDiskResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
	int user_request_id = request.header.user_request_id;

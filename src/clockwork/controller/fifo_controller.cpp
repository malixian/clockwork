...
FifoControllerImpl::FifoControllerImpl(int client_port, std::vector<std::pair<std::string, std::string>> worker_host_port_pairs) : Controller(client_port, worker_host_port_pairs), model_id_seed(0), action_id_seed(0) {
	current_worker = 0;
...
void FifoControllerImpl::save_callback(int id, std::function<void(std::shared_ptr<workerapi::Result>)> callback) {
...
std::function<void(std::shared_ptr<workerapi::Result>)> FifoControllerImpl::get_callback(int id) {
	std::lock_guard<std::mutex> lock(actions_mutex);
	auto it = action_callbacks.find(id);
...
void FifoControllerImpl::sendResult(std::shared_ptr<workerapi::Result> result) {
	auto callback = get_callback(result->id);
	CHECK(callback != nullptr) << "Received result without a corresponding action: " << result->str();
	callback(result);
}

void FifoControllerImpl::uploadModel(clientapi::UploadModelRequest &request, std::function<void(clientapi::UploadModelResponse&)> callback) {
...
void FifoControllerImpl::infer(clientapi::InferenceRequest &request, std::function<void(clientapi::InferenceResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
...
void FifoControllerImpl::evict(clientapi::EvictRequest &request, std::function<void(clientapi::EvictResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
...
/** This is a 'backdoor' API function for ease of experimentation */
void FifoControllerImpl::loadRemoteModel(clientapi::LoadModelFromRemoteDiskRequest &request, std::function<void(clientapi::LoadModelFromRemoteDiskResponse&)> callback) {
	std::cout << "Client  -> " << request.str() << std::endl;
	int user_request_id = request.header.user_request_id;

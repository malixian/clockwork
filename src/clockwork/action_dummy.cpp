...
LoadModelFromDiskActionDummy::LoadModelFromDiskTaskImpl::LoadModelFromDiskTaskImpl(LoadModelFromDiskActionDummy* load_model) : 
...
void LoadModelFromDiskActionDummy::LoadModelFromDiskTaskImpl::run(cudaStream_t stream) {
	try {
...
void LoadModelFromDiskActionDummy::LoadModelFromDiskTaskImpl::success(RuntimeModel* rm) {
...
void LoadModelFromDiskActionDummy::LoadModelFromDiskTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	load_model->handle_error(error);
}

LoadModelFromDiskActionDummy::LoadModelFromDiskActionDummy(ClockworkRuntimeDummy* runtime, 
	std::shared_ptr<workerapi::LoadModelFromDisk> action) :
	runtime(runtime), action(action), task(nullptr) {
}

LoadModelFromDiskActionDummy::~LoadModelFromDiskActionDummy() {
	if (task != nullptr) delete task;
	action = nullptr;
}

void LoadModelFromDiskActionDummy::submit() {
	CHECK(task == nullptr);
	task = new LoadModelFromDiskTaskImpl(this);
	runtime->load_model_executor->enqueue(task);
}

void LoadModelFromDiskActionDummy::handle_error(TaskError &error) {
...
LoadWeightsActionDummy::LoadWeightsTaskImpl::LoadWeightsTaskImpl(LoadWeightsActionDummy* load_weights):
...
void LoadWeightsActionDummy::LoadWeightsTaskImpl::run(cudaStream_t stream) {
	try {
		LoadWeightsTask::run(stream);
		load_weights->runtime->executors[gpu_id]->enqueueRun(this);
	} catch (TaskError &error) {
		load_weights->handle_error(error);
	}
}

void LoadWeightsActionDummy::LoadWeightsTaskImpl::process_completion() {
	try {
...
void LoadWeightsActionDummy::LoadWeightsTaskImpl::success(RuntimeModel* rm) {
...
void LoadWeightsActionDummy::LoadWeightsTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	load_weights->handle_error(error);
}

LoadWeightsActionDummy::LoadWeightsActionDummy(ClockworkRuntimeDummy* runtime, std::shared_ptr<workerapi::LoadWeights> action) :
	runtime(runtime), action(action), task(nullptr) {
}

LoadWeightsActionDummy::~LoadWeightsActionDummy() {
	if (task != nullptr) delete task;
	action = nullptr;
}

void LoadWeightsActionDummy::submit() {
	task = new LoadWeightsTaskImpl(this);
	runtime->executors[action->gpu_id]->enqueue(task);
}

void LoadWeightsActionDummy::handle_error(TaskError &error) {
	auto result = std::make_shared<workerapi::ErrorResult>();
...
EvictWeightsActionDummy::EvictWeightsTaskImpl::EvictWeightsTaskImpl(EvictWeightsActionDummy* evict_weights) : 
		EvictWeightsTask(
...
void EvictWeightsActionDummy::EvictWeightsTaskImpl::run(cudaStream_t stream) {
	try {
...
void EvictWeightsActionDummy::EvictWeightsTaskImpl::success(RuntimeModel* rm) {
...
void EvictWeightsActionDummy::EvictWeightsTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	evict_weights->handle_error(error);
}

EvictWeightsActionDummy::EvictWeightsActionDummy(ClockworkRuntimeDummy* runtime, std::shared_ptr<workerapi::EvictWeights> action) :
	runtime(runtime), action(action), task(nullptr) {
}

EvictWeightsActionDummy::~EvictWeightsActionDummy() {
	if (task != nullptr) delete task;
	action = nullptr;
}

void EvictWeightsActionDummy::submit() {
...
void EvictWeightsActionDummy::handle_error(TaskError &error) {
	auto result = std::make_shared<workerapi::ErrorResult>();
...
uint64_t InferActionDummy::copy_input_earliest() {
	return copy_input_lead_in > action->earliest ? 0 : (action->earliest - copy_input_lead_in);
}

InferActionDummy::CopyInputTaskImpl::CopyInputTaskImpl(InferActionDummy* infer):
...
void InferActionDummy::CopyInputTaskImpl::run(cudaStream_t stream) {
	try {
...
void InferActionDummy::CopyInputTaskImpl::process_completion() {
	try {
		CopyInputTask::process_completion();
	} catch (TaskError &error) {
		infer->handle_error(error);
...
void InferActionDummy::CopyInputTaskImpl::success(RuntimeModel* rm, char* io_memory) {
	infer->rm = rm;
	infer->io_memory = io_memory;
	infer->exec = new ExecTaskImpl(infer);
	infer->runtime->executors[gpu_id]->enqueue(infer->exec);
}

void InferActionDummy::CopyInputTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	infer->handle_error(error);
}

InferActionDummy::ExecTaskImpl::ExecTaskImpl(InferActionDummy* infer):
	ExecTask(infer->rm,
...
void InferActionDummy::ExecTaskImpl::run(cudaStream_t stream) {
	try {
		gpu_clock_before = infer->runtime->gpu_clock->get(gpu_id);
		ExecTask::run(stream);
		infer->runtime->executors[gpu_id]->enqueueRun(this);
	} catch (TaskError &error) {
		infer->handle_error(error);
	}
}

void InferActionDummy::ExecTaskImpl::process_completion() {
	try {
...
void InferActionDummy::ExecTaskImpl::success() {
	infer->copy_output = new CopyOutputTaskImpl(infer);
	infer->runtime->executors[gpu_id]->enqueue(infer->copy_output);
}

void InferActionDummy::ExecTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	infer->handle_error(error);
}

InferActionDummy::CopyOutputTaskImpl::CopyOutputTaskImpl(InferActionDummy* infer):
...
void InferActionDummy::CopyOutputTaskImpl::run(cudaStream_t stream) {
	try {
...
void InferActionDummy::CopyOutputTaskImpl::process_completion() {
	// try {
	// 	CopyOutputTask::process_completion();
	// } catch (TaskError &error) {
	// 	infer->handle_error(error);
...
void InferActionDummy::CopyOutputTaskImpl::success(char* output) {
	infer->handle_completion(output);
}

void InferActionDummy::CopyOutputTaskImpl::cancel() {
	TaskError error(actionCancelled, "Action cancelled");
	infer->handle_error(error);
}

InferActionDummy::InferActionDummy(ClockworkRuntimeDummy* runtime, std::shared_ptr<workerapi::Infer> action) :
		runtime(runtime), action(action), rm(nullptr), io_memory(nullptr), zero_size(action->input_size == 0) {}

InferActionDummy::~InferActionDummy() {
...
void InferActionDummy::submit() {
	copy_input = new CopyInputTaskImpl(this);
	runtime->executors[action->gpu_id]->enqueue(copy_input);
}

void InferActionDummy::handle_completion(char* output) {
	if (io_memory != nullptr) {
		runtime->manager->io_pools[action->gpu_id]->free(io_memory);
		io_memory = nullptr;
	}
...
void InferActionDummy::handle_error(TaskError &error) {
	if (io_memory != nullptr) {
		runtime->manager->io_pools[action->gpu_id]->free(io_memory);
		io_memory = nullptr;
	}

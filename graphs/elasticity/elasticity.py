...
file_name = "elasticity.csv"
output_file_name = "elasticity.pdf"
...
ax1.set_ylabel('latency / inf (ms)', fontname=FONT_FAMILY, fontsize=10)
ax2.set_ylabel('drop rate (inf/s)', fontname=FONT_FAMILY, fontsize=10)
ax3.set_ylabel('throughput (inf/s)', fontname=FONT_FAMILY, fontsize=10)
...
with open(file_name, 'r') as csvfile:
    ...
    for row in row_reader:
        ...
        if status == SUCCESSFUL:
            ...
            if (completion_second not in total_data["dropped"].keys()):
                total_data["dropped"][completion_second] = 0
            total_data["dropped"][completion_second] += 1
    ...
    for x in data.keys():        
        ...
        ax2.plot(ordered_dropped.keys(), ordered_dropped.values(),
                label="dropped_" + str(x), marker="", color=colorset[x][0] )
        ...
        ax1.plot(ordered_successful.keys(),np.divide(list(ordered_latency.values()), list(ordered_successful.values())), label="model_"+str(x), marker="", color=colorset[x][0], linestyle="-")
        ax1.set_ylim(0)
    ...
    if (len(data.keys()) > 1):        
        ...
        ordered_total_dropped = collections.OrderedDict(
            sorted(total_data["dropped"].items()))

        print(ordered_total_dropped)

        ax4.plot(ordered_total.keys(), ordered_total.values(),
        label="Total" , marker="", color="slategray", linestyle=":", alpha=0.6)
        ax2.plot(ordered_total_dropped.keys(), ordered_total_dropped.values(),
        label="Total" , marker="", color="slategray", linestyle=":", alpha=0.6)
        ...
        ax2.set_ylim(0, max(ordered_total.values())* 1.1)    
...
ax1.set_title("(d) Latency")
ax2.set_title("(c) Drop Rate")
ax3.set_title("(b) Throughput")
